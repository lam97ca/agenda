...
import os
#Para trabajar con archivos de texto formato json en python se debe importar el siguiente módulo
import json
...
def agenda_json(agenda_path):
        """La funcion genera archivo en .json"""
        agenda = descargar_agenda(agenda_path)
        datos_json = json.dumps(agenda)
    ...
    while(True):
		...
		#Menu de la agenda
		        ...
		        if opcion == "1":
		        	...
		        	agenda_json(agenda_path)
		        	time.sleep(4)
		        elif opcion == "7":
		        	crear_contacto(agenda_path)
		        	primero = open('agenda.txt', 'wt')
		        	primero.read()
		        	primer.close()
		        	segundo = open('agenda.txt', 'at')
		        	segundo.read()
		        	segundo.close()
		        	time.sleep(4)
		        elif opcion == "8":
		            print("He entrado en 8")   
		            break
		            #sale del menu
		        else:
